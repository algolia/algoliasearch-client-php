...
class ClickedFilters extends AbstractModel implements ModelInterface, \ArrayAccess, \JsonSerializable
{
    ...
    public function setAuthenticatedUserToken($authenticatedUserToken)
    {
        if (!is_null($authenticatedUserToken) && (mb_strlen($authenticatedUserToken) > 129)) {
            throw new \InvalidArgumentException('invalid length for $authenticatedUserToken when calling ClickedFilters., must be smaller than or equal to 129.');
        }
        if (!is_null($authenticatedUserToken) && (mb_strlen($authenticatedUserToken) < 1)) {
            throw new \InvalidArgumentException('invalid length for $authenticatedUserToken when calling ClickedFilters., must be bigger than or equal to 1.');
        }
        if (!is_null($authenticatedUserToken) && (!preg_match('/[a-zA-Z0-9_=\\/+-]{1,129}/', $authenticatedUserToken))) {
            throw new \InvalidArgumentException("invalid value for {$authenticatedUserToken} when calling ClickedFilters., must conform to the pattern /[a-zA-Z0-9_=\\/+-]{1,129}/.");
        }
